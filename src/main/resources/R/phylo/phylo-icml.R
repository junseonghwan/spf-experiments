...
folders<-c("10", "20", "40", "80", "160")
#folders<-c("20", "40")
method_type<-"spf"
N<-length(folders)
num_rep<-c(3)
dd<-matrix(0, ncol=N, nrow = num_rep)
hh<-matrix(0, ncol=N, nrow = num_rep)
logZ<-matrix(0, ncol=N, nrow = num_rep)
...
for (idx in num_rep)
{
  for (i in 1:N)
  {
    ...
    logZ[idx,i]<-read.csv(paste(dir, folder, "/output", idx, "/phylo-spf-logZ.csv", sep=""), header=F)$V1[2]
  }
}
#dev.off()
